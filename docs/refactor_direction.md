...
- 本文档仅承载“长期架构方向”；项目总路线图请以 `docs/roadmap.md` 为准。
...
- B1.5 多模态上下文自动关联（对齐 AstrBot）：
  - 目标：在不引用/不工具的情况下，触发消息需要媒体理解时自动附带相关图片/表情包；主上游不支持图片时走 caption 兜底。
  - 实施与验收口径见：`docs/roadmap.md` 的 P0 专题。
- B2. 历史上下文语义一致：
