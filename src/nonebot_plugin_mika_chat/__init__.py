...
if PluginMetadata is not None:
    __plugin_meta__ = PluginMetadata(
        name="mika_chat",
        description="基于 OneBot 协议的 Mika Chat 插件（v11/v12 best-effort 兼容）",
...
try:
    ...
    try:
        ...
        if STRICT_STARTUP or not _is_missing_dependency_error(exc):
            raise
        log.warning(f"mika_chat: matcher 注册失败，已跳过（可设置 MIKA_STRICT_STARTUP=1 强制失败）| error={exc}")
except Exception as exc:
    if STRICT_STARTUP or not _is_missing_dependency_error(exc):
        raise
    log.warning(f"mika_chat: 生命周期注册失败，已降级到最小模式（可设置 MIKA_STRICT_STARTUP=1 强制失败）| error={exc}")
