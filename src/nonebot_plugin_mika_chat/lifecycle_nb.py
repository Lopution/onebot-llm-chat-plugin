...
async def _probe_api_health_once(config: Config) -> Dict[str, Any]:
    ...
    base_url = str(llm_cfg.get("base_url") or config.gemini_base_url).rstrip(BASE_URL_RSTRIP_CHAR)
    model_name = str(llm_cfg.get("fast_model") or llm_cfg.get("model") or config.gemini_model)
...
async def on_bot_connect(bot: Bot):
    ...
    try:
        ...
        await sync_offline_messages(bot)
...
async def init_gemini():
    ...
    primary_key = str(llm_keys[0] if llm_keys else config.gemini_api_key)
    rotated_keys = llm_keys[1:] if len(llm_keys) > 1 else list(config.gemini_api_key_list)
    runtime_state.client = GeminiClient(
        api_key=primary_key,
        base_url=str(llm_cfg.get("base_url") or config.gemini_base_url),
        model=str(llm_cfg.get("model") or config.gemini_model),
