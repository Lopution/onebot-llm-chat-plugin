...
class TestMikaAPIExceptions:
    """测试自定义异常类"""
    
    def test_mika_api_error_basic(self):
        """测试基础 API 错误"""
        from mika_chat_core.mika_api import MikaAPIError
        
        error = MikaAPIError("Test error", status_code=400, retry_after=0)
    ...
    def test_rate_limit_error(self):
        """测试限流错误"""
        from mika_chat_core.mika_api import RateLimitError
    ...
    def test_authentication_error(self):
        """测试认证错误"""
        from mika_chat_core.mika_api import AuthenticationError
    ...
    def test_server_error(self):
        """测试服务器错误"""
        from mika_chat_core.mika_api import ServerError
...
class TestMikaClientInit:
    ...
    def test_init_with_defaults(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="test-key-123")
    ...
    def test_init_with_api_key_list(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            ...
            client = MikaClient(api_key="default-key", api_key_list=key_list)
    ...
    def test_init_with_persistent_storage_disabled(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", True):
            client = MikaClient(api_key="test-key", use_persistent_storage=False)
    ...
    def test_init_with_persistent_storage_unavailable(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="test-key", use_persistent_storage=True)
            
            # 即使要求使用持久化，但因为不可用所以应该是 False
            assert client._use_persistent == False
            assert client._context_store is None

    def test_init_with_persistent_storage_enabled_sets_contexts_fallback(self):
        """测试持久化模式下也会初始化内存后备上下文容器。"""
        from mika_chat_core.mika_api import MikaClient

        fake_store = AsyncMock()
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", True), patch(
            "mika_chat_core.mika_api.get_context_store",
            return_value=fake_store,
        ):
            client = MikaClient(api_key="test-key", use_persistent_storage=True)

            assert client._use_persistent == True
            assert client._context_store is fake_store
            assert hasattr(client, "_contexts")
            assert isinstance(client._contexts, dict)


class TestMikaClientAPIKeyRotation:
    """测试 API Key 轮询"""
    ...
    def test_get_api_key_single(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="single-key")
    ...
    def test_get_api_key_rotation(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            key_list = ["key1", "key2", "key3"]
            client = MikaClient(api_key="default", api_key_list=key_list)
...
class TestMikaClientContextManagement:
    """测试上下文管理（内存模式）"""
    ...
    def test_add_to_context_memory_mode(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="test-key", max_context=5)
    ...
    def test_context_truncation(self):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="test-key", max_context=2)
    ...
    @pytest.mark.asyncio
    async def test_add_message_falls_back_to_memory_when_store_write_fails(self):
        """持久化写入失败时，应显式降级到内存上下文。"""
        from mika_chat_core.mika_api import MikaClient
        from mika_chat_core.utils.context_store import ContextStoreWriteError

        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="test-key")

        failing_store = AsyncMock()
        failing_store.add_message = AsyncMock(
            side_effect=ContextStoreWriteError("db write failed")
        )

        client._use_persistent = True
        client._context_store = failing_store

        await client.add_message("user123", "user", "hello")

        key = client._get_context_key("user123", None)
        assert len(client._contexts[key]) == 1
        assert client._contexts[key][0]["content"] == "hello"
        failing_store.add_message.assert_awaited_once()


class TestMikaClientHTTPClient:
    """测试 HTTP 客户端管理"""
...
class TestMikaClientChat:
    ...
    async def test_chat_with_images(self, mock_httpx_client, mock_api_response_success):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False), patch(
            "mika_chat_core.mika_api.HAS_IMAGE_PROCESSOR",
            False,
        ):
            client = MikaClient(api_key="test-key")
    ...
    async def test_chat_authentication_error(self, mock_httpx_client):
        ...
        with patch("mika_chat_core.mika_api.HAS_SQLITE_STORE", False):
            client = MikaClient(api_key="invalid-key")
...
@pytest.mark.asyncio
class TestMikaClientAsyncContext:
    """测试异步上下文管理"""
...
class TestMikaClientToolHandlers:
    """测试工具处理器注册"""
