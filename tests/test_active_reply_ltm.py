...
async def test_check_proactive_disabled_by_ltm_gate():
    ...
    old_whitelist = list(getattr(m.plugin_config, "gemini_group_whitelist", []))
    try:
        m.plugin_config.gemini_active_reply_ltm_enabled = False
        ...
        m.plugin_config.gemini_group_whitelist = old_whitelist
...
async def test_check_proactive_blocked_by_active_reply_whitelist():
    ...
    try:
        ...
        m.plugin_config.gemini_active_reply_whitelist = ["999999"]
        m.plugin_config.gemini_proactive_keywords = ["Mika"]
        m.plugin_config.gemini_group_whitelist = []
